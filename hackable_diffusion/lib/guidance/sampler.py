...
class ConditionalDiffusionSampler:
  ...
  def _run_loop(
    ...
    # (e.g. PosteriorSamplerInferenceFn) need an rng to draw their
    # per-call noise; deterministic inference fns ignore it entirely.
    # Split only in the stochastic branch so the rng stream feeding the
    # scan body is bit-identical to the pre-fix behaviour for every
    # existing deterministic-inference-fn caller.
    if uses_rng:
